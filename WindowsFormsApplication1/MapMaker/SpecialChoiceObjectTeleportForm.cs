...
{
    public partial class SpecialChoiceObjectTeleportForm : Form
    {
        public SpecialChoiceObjectTeleportForm(Teleport t,Level currentlevel)
        {
            InitializeComponent();
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (_selectedgameobject != null)
            {
                ...
                try
                {
                    ...
                    if (!(_selectedgameobject is Teleport) || _selectedgameobject == _teleport)
                        throw new Exception();
        ...
        private void UpdateDrawLevel(Level level, Graphics g)
        {
            for (int i = 0; i < level.GameObjects.Count; i++)
                if (level.GameObjects[i].GameSprite.Image != null)
                {
                    ...
                    if (_teleport.Teleport2 == level.GameObjects[i])
                ...
                foreach (GameObject go in currentlevel.GameObjects)
                {
                    if (go.Position.Contains(p.X, p.Y))
                    {
                        try
                        {
                            ...
                            _teleport.Teleport2 = (Teleport)_selectedgameobject;
                        }
                        catch
                        {
                            _selectedgameobject = go;
            ...
            else if (mb == MouseButtons.Right)
            {
                ...
                foreach (GameObject go in currentlevel.GameObjects)
                {
                    if (go.Position.Contains(p.X, p.Y))
                    {
                        ...
                        if (_teleport.Teleport2 == _selectedgameobject)
                            _teleport.Teleport2 = null;
                        break;
                        //finded = true;
                        //break;
                    }
